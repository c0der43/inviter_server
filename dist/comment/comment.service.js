...
let CommentService = class CommentService {
    ...
    async createComment(dto, userId) {
    ...
    async deleteMyComment(userId, commentId) {
        ...
        if (findMe.id !== comment.user.id) {
            throw new common_1.HttpException('Удалить комментарий может только пользователь, который добавил этот коментарий!', common_1.HttpStatus.NOT_FOUND);
        }
    ...
    async getCommentById(commentId) {
        const findComment = await this.repository
            ...
            .getOne();
        if (!findComment) {
            throw new common_1.HttpException('Не удалось найти комментарий с таким id!', common_1.HttpStatus.NOT_FOUND);
        }
        return findComment;
    }
    async addOneLikesComment(commentId) {
